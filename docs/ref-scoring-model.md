...
|---|---|---|
| Precision@50 | ≥ 0.60 contractual gate; ≥ 0.68 demonstrated technical ceiling (multi-region CI gate) | Contractual acceptance gate. Technical ceiling: achieved on public-data backtest; enforced as CI regression gate. Single-region floor: ≥ 0.25 (structural ceiling ≈ 0.333 with Singapore-only labels). |
...
This is a network-propagation prediction: it identifies who is most likely to be designated next, given a confirmed seed, before any regulatory action occurs. See [docs/backtracking-runbook.md](https://github.com/edgesentry/indago/blob/main/.agents/skills/indago-run-backtrack/references/backtracking-runbook.md) for operation instructions.

...
Custom feeds are CSV files dropped in `_inputs/custom_feeds/`. Files ending in `_sample` or `.gitkeep` are never uploaded. See [pipeline-operations.md](https://github.com/edgesentry/indago/blob/main/.agents/skills/indago-run-pipeline/references/pipeline-operations.md#custom-feed-drop-ins) for the full file format reference.

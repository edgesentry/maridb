...
<div class="card">
  ...
  <p>The model scores each vessel 0–1 (<strong>Conf</strong>) based on AIS behaviour anomalies.
     <strong>SD</strong> (sanctions distance) shows how many hops away the vessel is from a sanctioned entity —
     <strong>SD=0 means directly sanctioned</strong>; SD=99 means no sanctions link was found.</p>
  <table>
    <tr><th>Section</th><th>What it means</th><th>Next action</th></tr>
    <tr>
      <td><strong>sanctions_distance=0</strong></td>
      <td>Vessel (or its operator) is on the OFAC SDN list</td>
      <td>Open MT + VF links to check current position &amp; AIS status.
          For named vessels use the OFAC link to confirm the SDN entry matches the IMO number.
          Escalate immediately if the vessel is underway or recently port-called.</td>
    </tr>
    <tr>
      <td><strong>Stateless MMSIs ⚠</strong></td>
      <td>MMSI prefix is not allocated by ITU — likely a spoofed or cloned identity</td>
      <td>Cross-check the broadcast name on MarineTraffic against the ITU MMSI database.
          Flag for further identity verification before any commercial engagement.</td>
    </tr>
    <tr>
      <td><strong>Full top-{top_n} watchlist</strong></td>
      <td>Remaining high-confidence candidates without a direct sanctions hit</td>
      <td>Review vessels with Conf ≥ 0.5 and any with recent flag or name changes.
          Use MT voyage history to look for dark periods, ship-to-ship transfers, or unusual port calls.</td>
    </tr>
  </table>
  <p style="margin-top:12px;font-size:12px;color:#666;">
    Links: <strong>MT</strong> = MarineTraffic &nbsp;·&nbsp; <strong>VF</strong> = VesselFinder &nbsp;·&nbsp;
    <strong>OFAC</strong> = U.S. Treasury SDN search (appears only when a vessel name is known).
    Report generated by EdgeSentry indago · <a href="{_e(run_url)}">CI run →</a>
  </p>
</div>"""

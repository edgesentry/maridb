...
Usage:
    ...
    # Checkpoint + push public catalog to maridb-public
...
_DEFAULT_DATA_DIR = os.getenv("MARIDB_DATA_DIR", str(Path.home() / ".arktrace" / "data"))
...
def run(
    ...
    # Build DataFrames
    # ------------------------------------------------------------------
    from pipelines.storage.ducklake import checkpoint as dl_checkpoint
    from pipelines.storage.ducklake import write_table
    ...
    if push_public:
        print("Pushing public DuckLake catalog to maridb-public ...")
        ret = subprocess.run(
...
def main() -> int:
    ...
    parser.add_argument(
        "--push-public",
        action="store_true",
        help="After checkpointing, push public catalog to maridb-public via sync_r2.py",
    )
    parser.add_argument(
        "--push-private",
        action="store_true",
